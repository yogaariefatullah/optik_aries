    ...
    <div class="card mb-5 mb-xl-8">
        <div class="card-header border-0 pt-5">
            <h3 class="card-title align-items-start flex-column">
                <span class="card-label fw-bold fs-3 mb-1">List Digitalisasi Arsip Video    </span>
            </h3>
            <div class="card-toolbar">
                <a href="{!! route('arsip.video.create') !!}"><button type="button" data-bs-toggle="modal" data-bs-target=""
        ...
        <div class="card-body py-3">
            <div class="table-responsive">
                ...
                <table class="table align-middle gs-0 gy-4">
                    ...
                    <tbody>
                        @if(count($arsip_video) == 0)
                            <tr>Data tidak ada</tr>
                        @else
                            @foreach($arsip_video as $key => $val)
                                <tr>
                                    <td>
                                    {{ ($arsip_video->currentPage() - 1) * $arsip_video->perPage() + $loop->index + 1 }}
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-end">
                                        <a href="{{route('arsip.video.edit',$val->id)}}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <form action="{{ route('arsip.video.destroy', $val->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                ...
                {{ $arsip_video->onEachSide(1)->appends(array_merge(request()->query(), ['page' => $arsip_video->currentPage()]))->links('pagination::bootstrap-5') }}
            </div>
        </div>
    </div>
