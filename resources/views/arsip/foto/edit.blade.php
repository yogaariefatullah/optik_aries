...
<div class="class">
    ...
    <div class="card mb-5 mb-xl-8 custom-form-input">
        ...
        {{-- <div class="card-body py-3">
            ...
            <!--begin::Form-->
        <form  method="POST" action="{{ route('arsip.foto.update',$arsip->id) }}" enctype='multipart/form-data'>
            @csrf
                    ...
                    <div class="form-group col-6">
                        ...
                        <select name="id_subjek" aria-label="Select a Country" data-control="select2" data-placeholder="Pilih subject..." class="form-select form-select-solid form-select-lg fw-semibold">
                            ...
                            @foreach($subject as $key => $value)
                                <option value="{{$value->id}}" @if($arsip->id_subjek == $value->id) selected="selected"@endif>{{$value->subjek}}</option>
                            @endforeach
                        </select>
                    </div>
                            ...
                            @foreach($arsip_file as $key => $val)
                                <tr>
                                    ...
                                    <div class="symbol symbol-500 mr-5 align-self-start align-self-xxl-center">
                                        <div class="symbol-label" style="background-image:url('{{asset("uploads/arsip-foto/".$val->file)}}')"></div>
...
@section('javascript')
    <script>
        ...
        function hapusdata(id){
            let url = '{{ route("arsip.foto.destroy.file", "id") }}';
        ...
        $('#kt_dropzonejs_example_1').dropzone({
                url: '{{ route("arsip.foto.upload.edit",$arsip->id) }}', // Set the url for your upload script location
        ...
        function refreshTabel() {
            let id = $('#arsipid').val();
            let url = '{{ route("arsip.foto.edit.refresh", "id") }}';
            url = url.replace('id', id);
            $.ajax({
                url: url,
            method: 'GET',
