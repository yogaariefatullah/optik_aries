    ...
    <div class="card mb-5 mb-xl-8">
        <div class="card-header border-0 pt-5">
            ...
            <div class="card-toolbar">
                <a href="{!! route('pustaka.audio.create') !!}"><button type="button" data-bs-toggle="modal" data-bs-target=""
        ...
        <div class="card-body py-3">
            <div class="table-responsive">
                <div class="row mb-3">
                    <div class="col">
                        <form action="{{ route('pustaka.audio.index') }}" method="GET">
                ...
                <table class="table align-middle gs-0 gy-4">
                    ...
                    <tbody>
                        @if(count($pustaka_audio) == 0)
                            <tr>Data tidak ada</tr>
                        @else
                            @foreach($pustaka_audio as $key => $val)
                                <tr>
                                    <td>
                                    {{ ($pustaka_audio->currentPage() - 1) * $pustaka_audio->perPage() + $loop->index + 1 }}
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-end">
                                        <a href="{{route('pustaka.audio.edit',$val->id)}}" class="btn btn-icon btn-bg-light btn-active-color-primary btn-sm me-1">
                                        ...
                                        <form id="postForm" action="{{ route("pustaka.audio.detail") }}" method="POST" style="display: none;">
                                            @csrf
                                    ...
                                    <div class="modal-dialog" role="document">
                                        <div class="modal-content">
                                            ...
                                            <div class="modal-footer">
                                                ...
                                                <form action="{{ route('pustaka.audio.destroy', $val->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                ...
                {{ $pustaka_audio->onEachSide(1)->appends(array_merge(request()->query(), ['page' => $pustaka_audio->currentPage()]))->links('pagination::bootstrap-5') }}
            </div>
        </div>
    </div>
