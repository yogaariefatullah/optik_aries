@php
    $menu = getAllMenu();
@endphp
<div id="kt_aside" style="background-color:white!important;" class="aside pt-7 pb-4 pb-lg-7 pt-lg-17" data-kt-drawer="true"
    data-kt-drawer-name="aside" data-kt-drawer-activate="{default: true, lg: 0}" data-kt-drawer-overlay="true"
    ...
    <div class="aside-menu flex-column-fluid ps-3 ps-lg-5 pe-1 mb-9" id="kt_aside_menu">
        <div class="w-100 hover-scroll-y pe-2 me-2" id="kt_aside_menu_wrapper" data-kt-scroll="true"
            data-kt-scroll-activate="{default: 0, lg: true}" data-kt-scroll-height="auto"
                ...
                @foreach ($menu as $value)
                    ...
                    @if ($check)
                        @if ($value->url != '#' && $value->parent_id == 0)
                            @if ($check->r != null || $check->r != 0)
                                <div data-kt-menu-trigger="click"
                                    class="menu-item menu-accordion {{ Str::contains(request()->url(), $value->url) ? 'show' : '' }}">
                                    <div class="menu-item">
                                        <a class="{{ Str::contains(request()->url(), $value->url) ? 'menu-link active' : 'menu-link' }}"
                                            href="{{ url($value->url) }}">
                                            {{ $value->name }}
                                        </a>
                                    </div>
                        ...
                        @elseif($value->url == '#' && $value->parent_id == 0)
                            ...
                            @if ($check->r != null || $check->r != 0)
                                <div data-kt-menu-trigger="click"
                                    ...
                                    <div
                                        class="menu-sub menu-sub-accordion {{ Str::contains(request()->url(), $value->url) ? 'show' : '' }}">
                                        @foreach ($Child as $value_child)
                                            <?php $check_child = getPermission(Auth::user()->group_id, $value_child->id); ?>
                                            @if ($check_child->r != null || $check_child->r != 0)
                                                <div class="menu-item">
                                                    <a class="{{ Str::contains(request()->url(), $value_child->url) ? 'menu-link active' : 'menu-link' }}"
                                                        href="{{ url($value_child->url) }}">
                                                        {{ $value_child->name }}
                                                    </a>
                                                </div>
                                            @endif
